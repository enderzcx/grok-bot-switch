...
- “使用”和“切回上一通道”自动先检查、后执行，不弹常规确认框。主机未接入、忙碌、存在待处理命令或模拟 apply 未启用时不执行；卡片或工具栏就地显示原因，允许重试。删除供应商、移除密钥仍保留确认框。
...
- 初版计划弹窗已改为卡片内显示“主机未接入”，不会提交 apply；不显示切换弹窗。
...
- 移除常规切换/切回的计划弹窗，主按钮改为“使用”；自动调用既有 dry-run 后才 apply，后端 CSRF、dry-run gate 和忙碌检查未修改。
- 请求期间显示“切换中”并防止重复提交；只有刷新状态中的 activeProfile 匹配且无 blocker 才提示成功。模拟结果仍明确标注模拟，不显示真实“使用中”。
- 切换错误显示在对应卡片，切回错误显示在工具栏下方；删除和移除密钥确认保留。
- 本轮 React 23 项、Python 195 项、协议 Node 91 项测试通过；TypeScript/Vite 构建及 diff whitespace 检查通过。
- 本地浏览器验证未接入主机时卡片提示、无切换弹窗、可重复尝试、删除确认后取消；390px 宽度无横向溢出。未执行真实切换或调用付费推理。
